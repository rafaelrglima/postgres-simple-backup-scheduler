...
sudo git clone https://github.com/rafaelrglima/postgres-simple-backup-scheduler.git /var/backups/postgres-simple-backup-scheduler
cd /var/backups/postgres-simple-backup-scheduler
...
```bash
# PostgreSQL Backups - /var/backups/postgres-simple-backup-scheduler
# Hourly (every hour, retain 7 days), Weekly (Sunday, retain 30 days), Monthly (1st, retain 1 year)
0 * * * * /var/backups/postgres-simple-backup-scheduler/backup.sh hourly 7 >/dev/null 2>&1
0 0 * * 0 /var/backups/postgres-simple-backup-scheduler/backup.sh weekly 30 >/dev/null 2>&1
0 0 1 * * /var/backups/postgres-simple-backup-scheduler/backup.sh monthly 365 >/dev/null 2>&1
```
...
| Path | Purpose | User |
|------|---------|------|
| `/var/backups/postgres-simple-backup-scheduler` | **Recommended** - System backups location | root |
| `/opt/postgres-simple-backup-scheduler` | Alternative for optional software | root |
| `~/postgres-simple-backup-scheduler` | Personal/development use | regular user |

**Best Practice:** Use `/var/backups/postgres-simple-backup-scheduler` for production servers as `/var/backups/` is the standard Linux location for system backup files.
...
# Script location: /var/backups/postgres-simple-backup-scheduler/backup.sh
...
0 2 * * * /var/backups/postgres-simple-backup-scheduler/backup.sh daily 30 >/dev/null 2>&1
```

**Aggressive Backup Strategy (More frequent):**
```bash
# Every 15 minutes, keep 3 days
*/15 * * * * /var/backups/postgres-simple-backup-scheduler/backup.sh frequent 3 >/dev/null 2>&1

# Daily at 2 AM, keep 60 days
0 2 * * * /var/backups/postgres-simple-backup-scheduler/backup.sh daily 60 >/dev/null 2>&1

# Weekly on Sunday, keep 180 days
0 0 * * 0 /var/backups/postgres-simple-backup-scheduler/backup.sh weekly 180 >/dev/null 2>&1

# Monthly on 1st, keep 730 days (2 years)
0 0 1 * * /var/backups/postgres-simple-backup-scheduler/backup.sh monthly 730 >/dev/null 2>&1
```

**Conservative Setup (Less frequent, longer retention):**
```bash
# Daily at 3 AM, keep 90 days
0 3 * * * /var/backups/postgres-simple-backup-scheduler/backup.sh daily 90 >/dev/null 2>&1

# Monthly on 1st at midnight, keep 5 years
0 0 1 * * /var/backups/postgres-simple-backup-scheduler/backup.sh monthly 1825 >/dev/null 2>&1
```
...
[Service]
Type=oneshot
ExecStart=/var/backups/postgres-simple-backup-scheduler/backup.sh daily 30
User=root
```
...
```
/var/backups/postgres-simple-backup-scheduler/
...
- Check log files regularly: `tail -f /var/backups/postgres-simple-backup-scheduler/daily/*.log`
- Monitor disk space: `df -h /var/backups/postgres-simple-backup-scheduler/`
...
chmod 700 /var/backups/postgres-simple-backup-scheduler/backup.sh

# Credentials readable by root only  
chmod 600 /var/backups/postgres-simple-backup-scheduler/.env

# Backup directories (created automatically with these permissions)
chmod 700 /var/backups/postgres-simple-backup-scheduler/hourly/
chmod 700 /var/backups/postgres-simple-backup-scheduler/daily/
chmod 700 /var/backups/postgres-simple-backup-scheduler/monthly/
```
...
# Good
*/30 * * * * /var/backups/postgres-simple-backup-scheduler/backup.sh hourly 7
```
...
# Run backup
/var/backups/postgres-simple-backup-scheduler/backup.sh custom 14
